...
const int INVENTORY_COLS = 9;
const int TOTAL_SLOTS = INVENTORY_ROWS * INVENTORY_COLS;
...
bool IsTileBlocked(TileType type) {
	return (type == TILE_WATER);
}
bool showInventory = false;
int hotbarSlots = 5;

#define SLOT_SIZE 48 // Example size
...
void Render() {
	for (int i = 0; i < WORLD_H; i++) {
		for (int j = 0; j < WORLD_W; j++) {
			...
			case TILE_SAND: texture_index_x = 13; texture_index_y = 1; break;
...
void GenerateWorld() {
	float scale = 0.01f; // controls smoothness
...
void Begin() {
	Image tilemap = LoadImage("Tilemap.png");
...
int main(void)
{
	...
	SetWindowTitle("C++ JATEK");
	SetTargetFPS(30);
	player.x = screen_width / 2;
	player.y = screen_height / 2;
	player.speed = 2;
	Aseprite test = LoadAseprite("Player.aseprite");
...
#pragma endregion
	UnloadAseprite(test);
	UnloadTexture(textures[TEXTURE_TILEMAP]);
	CloseWindow();
